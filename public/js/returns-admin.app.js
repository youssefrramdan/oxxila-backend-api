  ...
  try {
    ...
    if (orderId)
      returns = returns.filter(
        (r) => String(r.order?._id || r.order) === orderId,
      );
    renderList();
    if (selected) {
...
function renderList() {
  ...
  if (!returns.length) {
    body.innerHTML =
      '<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:24px">No returns</td></tr>';
    return;
  }
...
function renderDetail() {
  ...
  if (r.refundStatus === "pending") {
    ...
    nextActions.push(
      `<button class="btn btn-secondary" onclick="patchStatus('rejected')">Reject</button>`,
    );
  }
  if (r.refundStatus === "approved") {
    if (r.logisticsHandler === "bosta" && !r.bostaExternalId) {
      nextActions.push(
        `<button class="btn btn-primary" onclick="scheduleBosta()">Schedule Bosta</button>`,
      );
    }
    nextActions.push(
      `<button class="btn btn-secondary" onclick="patchStatus('picked_up')">Mark picked up</button>`,
    );
  }
  if (r.refundStatus === "picked_up") {
    nextActions.push(
      `<button class="btn btn-secondary" onclick="patchStatus('received')">Mark received</button>`,
    );
  }
  if (r.refundStatus === "received") {
    nextActions.push(
      `<button class="btn btn-primary" onclick="openRefund()">Refund</button>`,
    );
  }
...
async function loadBostaCarriers() {
  try {
    ...
    bostaCarriers = (data || []).filter(
      (c) => c.apiProvider === "bosta" && c.type === "api",
    );
    ...
    sel.innerHTML = bostaCarriers
      ...
      .join("");
    if (
      bostaCarriers.length &&
      document.getElementById("approve-handler").value === "bosta"
    ) {
...
async function loadDropoffs(carrierId) {
  ...
  const { data } = await api(
    "GET",
    "/admin/carriers/" + carrierId + "/pickups",
  );
...
function onHandlerChange() {
  ...
  if (isBosta && bostaCarriers.length) {
    const carrierId =
      document.getElementById("approve-carrier").value || bostaCarriers[0]._id;
...
function init() {
  ...
  if (localStorage.getItem("oxxila_api_base"))
    base.value = localStorage.getItem("oxxila_api_base");
