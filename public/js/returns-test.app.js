...
const BASE = () =>
  (
    document.getElementById("base-url")?.value || "http://localhost:3000/api/v1"
  ).replace(/\/$/, "");
...
function switchTab(tab) {
  ...
  document
    ...
    .classList.toggle("hidden", tab !== "list");
...
async function loadMyReturns() {
  ...
  try {
    const { data } = await api(
      "GET",
      "/returns/my-returns?limit=20&sort=-createdAt",
    );
...
async function loadEligible(page = 1) {
  ...
  try {
    const res = await api(
      "GET",
      `/returns/eligible-orders?limit=50&page=${page}&sort=-deliveredAt`,
    );
    eligibleOrders =
      page === 1 ? res.data || [] : [...eligibleOrders, ...(res.data || [])];
    ...
    sel.innerHTML =
      ...
      eligibleOrders
        .map(
          (o) =>
            `<option value="${o._id}">#${String(o._id).slice(-6)} — ${o.totalPrice} EGP</option>`,
        )
        .join("");
  } catch (e) {
    toast(e.message, true);
...
async function onCountryChange() {
  ...
  const { data } = await api(
    "GET",
    `/shipping/countries/${countryId}/governorates`,
    null,
    true,
  );
  governorates = data || [];
...
async function submitReturn() {
  ...
  const gov = governorates.find(
    (g) => g._id === document.getElementById("governorate").value,
  );
