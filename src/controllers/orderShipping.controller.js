...
export const getOrderShippingDetail = asyncHandler(async (req, res, next) => {
  ...
  const carriers = await Carrier.find({ isActive: true }).sort({
    type: 1,
    name: 1,
  });
  ...
  if (order.shippingAddress.districtId) {
    districtBosta = await District.findById(
      order.shippingAddress.districtId,
    ).select("bostaApiCovered bostaDistrictId name");
  }
  ...
  const data = {
    ...
    carriers: carriers.map((c) => ({
      ...c.toObject(),
      coversGovernorate: coveredIds.some(
        (id) => id.toString() === c._id.toString(),
      ),
      hasDefaultPickup:
        c.apiProvider !== "bosta" ||
        c.type !== "api" ||
        bostaPickupCarrierIds.has(c._id.toString()),
    })),
  };
...
export const assignOrderShipping = asyncHandler(async (req, res, next) => {
  ...
  if (!carrier)
    return next(
      new ApiError(`No carrier found with id: ${req.body.carrierId}`, 404),
    );
  if (!carrier.isActive)
    return next(new ApiError("Carrier is not active", 400));
  ...
  try {
    const updated = await assignOrderToCarrier(order, carrier, req.user._id, {
      ...
      size: req.body.size, // ← الإضافة الوحيدة
    });
